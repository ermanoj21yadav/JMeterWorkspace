...
import org.apache.jmeter.threads.dynamic.AddThreadsFileWatcherImpl;
import org.apache.jmeter.threads.dynamic.KillThreadsFileWatcherImpl;
...
public class ThreadGroup extends AbstractThreadGroup {
    ...
    public void start(int groupCount, ListenerNotifier notifier, ListedHashTree threadGroupTree, StandardJMeterEngine engine) {
       ...
       /* String strBaseDir = FileServer.getFileServer().getBaseDir();
      
        
        AddThreadsFileWatcherImpl.watch(this, groupCount, notifier, threadGroupTree, engine, new File(strBaseDir + "/AddThreads.txt"));
        KillThreadsFileWatcherImpl.watch(this, groupCount, notifier, threadGroupTree, engine, new File(strBaseDir + "/KillThreads.txt"));*/
    }
    
    public void addThreads(int groupCount, ListenerNotifier notifier, ListedHashTree threadGroupTree, StandardJMeterEngine engine, int nAddThreads) {
        if(running == true)
        {
	        int numThreads = getNumThreads();
	        int rampUpPeriodInSeconds = getRampUp();
	        float perThreadDelayInMillis = ((float) (rampUpPeriodInSeconds * 1000) / (float) nAddThreads);
	
	        delayedStartup = isDelayedStartup(); // Fetch once; needs to stay constant
	        log.info("Adding thread group number " + groupCount
	                + " threads " + nAddThreads
	                + " ramp-up " + rampUpPeriodInSeconds
	                + " perThread " + perThreadDelayInMillis
	                + " delayedStart=" + delayedStartup);
	            ...
	            long now = System.currentTimeMillis(); // needs to be same time for all threads in the group
	            final JMeterContext context = JMeterContextService.getContext();
	            for (int i = numThreads; running && i < numThreads + nAddThreads; i++) {
	                JMeterThread jmThread = makeThread(groupCount, notifier, threadGroupTree, engine, i, context);
	                scheduleThread(jmThread, now); // set start and end time
	                jmThread.setInitialDelay((int)(i * perThreadDelayInMillis));
	                Thread newThread = new Thread(jmThread, jmThread.getThreadName());
	        ...
	        setNumThreads(numThreads + nAddThreads);
	        log.info("Started thread group number "+groupCount);
        }
    ...
    public Map<JMeterThread, Thread> getAllRunningThreads()
    {
    	return allThreads;
    }
